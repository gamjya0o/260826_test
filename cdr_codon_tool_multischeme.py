...
def is_header_line(line):
    """Check if a line is a header, even if it lacks a '>' prefix."""
    if line.startswith(">") or line.endswith(">>"):
        return True
    
    # 한글이나 숫자가 포함되어 있으면 제목으로 간주
    if re.search(r'[가-힣\d]', line):
        return True
        
    # 아미노산이 아닌 알파벳(b, j, o, u, x, z)이 포함되어 있으면 제목으로 간주 (예: mUtant, nUcleOtide)
    if re.search(r'[bjouxz]', line.lower()):
        return True
        
    # 짧은 줄이면서 특정 키워드가 포함되어 있으면 제목으로 간주
    if len(line) < 60:
        lower_line = line.lower()
        if "wild" in lower_line or "fasta" in lower_line or "clone" in lower_line:
            return True
...
def parse_records(content):
    ...
    for raw in content.splitlines():
        ...
        if is_header_line(line):
            if cur_header is not None or cur_seq:
